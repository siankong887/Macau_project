...
def build_observation_vector(
    ...
    """
    #这里只提取传入的vechile_type那个参数指定的车型
    """
    # vehicle_type = "car" 时
vehicle_data = {
    0: [120.0, 95.0],   # gate全局索引0，两个时段的流量
    1: [80.0, 70.0],    # gate全局索引1
    2: [200.0, 180.0],  # gate全局索引2
    ...
}
    """
    ...
    #创建一个全零数组，长度等于gate的数量
    g = np.zeros(len(sorted_gates))
    n_periods = 0

    """
    正常for循环访问字典只返回key
    d = {0: [120.0, 95.0], 1: [80.0, 70.0], 2: [200.0, 180.0]}

for item in d:
    print(item)
# 输出: 0, 1, 2  （只有 key）
    如果需要同时拿到key,value，需要用.items()
    for k, v in d.items():
    print(k, v)
# 0 [120.0, 95.0]
# 1 [80.0, 70.0]
    ...
    for idx, gate_index in enumerate(sorted_gates):
        ...
        n_periods = max(n_periods, len(counts))  # 记录最大时段数，也就是哪一个门有最多时段记录

    # 对角协方差矩阵：方差 = max(均值, min_variance)
    # 这是基于泊松分布 Var≈Mean 的近似，流量大的 gate 允许更大的噪声
    """
    np.diag()方法的作用是将一个一维数组变成一个二维的对角数组，对角线为方差，其余位置全为0
    这里隐含的假设观测噪声在每一个gate间是独立的
    """
...
def aggregate_to_edge_level(
    ...
    return aggregate_to_group_level(
        gate_observations=gate_observations,
        gate_idx_to_group=gate_idx_to_edge,
        min_variance=min_variance,
    )


def aggregate_to_group_level(
    gate_observations: ObservationData,
    gate_idx_to_group: dict[int, object],
    min_variance: float = 1.0,
) -> ObservationData:
    """按给定 group key 对 gate 级观测做聚合，并保留每组第一个 gate_index 作为代表索引。"""

    # edge_positions: edge → 该 edge 下各 gate 在 gate_observations.g 中的位置索引
    # edge_gate_indices: edge → 该 edge 下各 gate 的全局索引
    edge_positions: dict[object, list[int]] = {}
    edge_gate_indices: dict[object, list[int]] = {}

    for pos, gate_index in enumerate(gate_observations.gate_indices):
        group_key = gate_idx_to_group.get(gate_index)
        if group_key is None:
            # 该 gate 没有映射到任何路网边，跳过（不参与贝叶斯更新）
            continue
        edge_positions.setdefault(group_key, []).append(pos)
        edge_gate_indices.setdefault(group_key, []).append(gate_index)
...
def build_turning_observations(
    ...
    #第一层循环，得到单一的camera_id(摄像头名称)，camera_gates(一个列表，包含这个摄像头所有的gate)
    for camera_id, camera_gates in cameras_gates.items():
        ...
        for gate in camera_gates:
            ...
            # 取 edge 的 (起点, 终点) 作为键，忽略平行边的 key
            edge_key = (gate.edge[0], gate.edge[1])
            # 将当前 gate 的全局索引归入它所属的 edge 分组
            #起始点的元组作为key，value是一个列表，列表中的元素是在这条道路上的gate绝对索引序号
            #数据大致上长这样edge_groups:{(1,3):[0,1],(5,6):[3,4,6]}
            edge_groups.setdefault(edge_key, []).append(gate.gate_index)

            # 取该 gate 在所有时段的平均流量，累加到所属 edge 的总计数上
            """
            假设某个摄像头路口有3个gate
            gateA和gateB在edge(133,20)上，流量分别为20，23
            gateC在(22,22)上，流量为22
            则edge_counts变量大概长这样：
            edge_counts={(133,20):43,(22,22):22}
            """
            edge_counts[edge_key] = edge_counts.get(edge_key, 0.0) + float(
                np.mean(vehicle_data[gate.gate_index])
            )

        # 转向比例至少需要 2 个不同方向的 edge 才有意义，否则跳过该路口
        if len(edge_groups) < 2:
            continue

        # 将 edge 排序，保证后续向量的顺序一致
        """
        python 对tuple的排序，先比较第一个元素，如果相同再比较第二个元素
        要注意sorted函数对字典排序，只返回key的列表，不包含value，
        所以edge_keys大致长这样：[(12,3),(3,4)....]
        """
        edge_keys = sorted(edge_groups.keys())
        # 按排序后的 edge 顺序，构建各方向的流量数组
        observed_counts = np.array([edge_counts[key] for key in edge_keys], dtype=float)
        # 计算该路口所有方向的流量总和
        total = float(np.sum(observed_counts))

        # 总流量太低说明观测样本不足，统计不可靠，跳过
...
"""
总揽：
第一步，将原始数据聚合成为一个列表，并得到gate到全局索引的正向索引，以及全局索引到gate的反向索引
raw_bos 按照不同类型车辆分类，提取不同gate里，特定类别车辆的数量，每一个位置代表不同的时段：
raw_obs 数据大致长这样：
    raw_obs = {
    "car": {
        0: [120.0, 95.0],   # 全局索引0的gate，两个时段的car流量
...
第二步主要还是在整理各个gate的车流量数据，将特定类别的车辆，也就是传入的vehicle tye，各个时段的车流量
求得各个gate上改类车的均值，一个假设就是各个时间段的流量特征是一样的，很明显实验数据是不满足的
所以后面还要改，然后假设车辆计数过程是一个柏松过程，所以车辆的计数满足柏松分布，因此车辆计数的方差
就等于均值，看下输出：
    return ObservationData(
        g=g,
        sigma=sigma,
...
g是一个列表，列表上位置和gate的全局索引是完全对应的，比如说car这个类别的车辆的某个gate全局索引是0，这个gate
各个时段的均值就在列表0索引这个位置
sigma：二维数组，对角矩阵，主对角线元素是各个gate的统计流量方差
vehicle type:传入的那个车辆类型
n_time_periods:指定类型车辆中，有最多统计的时段有多少个时段
total_flow:指定类型车辆所有gate均值的加总，也就是一个时间段，平均来说总共有多少辆指定类型的车辆
"""